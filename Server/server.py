...
def sendTextMessage(client_name, message):
    ...
    other_client_socket.send(finla_message.encode())


def handleErrorMessage(client):
    message = '''
    You need to connect with one of the client first before sending any messgage.
...
def ftp():
    ...
    authorizer.add_user("lftpd","lftp",".",perm="erladfmw")
